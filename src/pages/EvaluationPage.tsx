...
const EvaluationPage = () => {
    ...
    if (loading && students.length === 0) {
        return <div className="text-center py-12 dark:text-white">Cargando...</div>;
    }
    ...
    if (!currentClassroom || !currentMatrix) {
        return <div className="text-center py-12 text-neutral-600 dark:text-neutral-400">Clase o Matriz de Evaluación no encontrada.</div>;
    }

    return (
        <div className="min-h-full space-y-4 p-2 sm:p-6 bg-neutral-50 dark:bg-dark-bg-soft">
            {/* Información del Aula */}
            <div className="bg-white dark:bg-dark-bg-card border border-neutral-200 dark:border-dark-border shadow-sm p-3 mb-3">
                <div className="flex items-center space-x-3">
                    <div className="bg-gradient-to-br from-primary-100 to-primary-200 dark:from-primary-800 dark:to-primary-900 p-2.5 rounded-lg">
                        <ClipboardList className="h-5 w-5 text-primary-600 dark:text-primary-300" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <h2 className="font-semibold text-neutral-900 dark:text-dark-text-primary truncate text-base sm:text-lg">
                                ...
                                <div className={`w-7 md:w-10 border-r border-black dark:border-neutral-600 ${cellClass} sticky left-8 z-0 flex-shrink-0`}>
                                    <button
                                        className={`
                                            ...
                                            ${currentParticipationLevel ? 'bg-primary-600 text-white' : 'text-neutral-300 dark:text-neutral-600 hover:bg-neutral-100 dark:hover:bg-neutral-700'}
                                        `}
                                                    ...
                                                    {['C', 'B', 'A', 'AD'].map((level) => {
                                                        ...
                                                        return (
                                                            <button
                                                                key={level}
                                                                className={`
                                                                    h-full border-r border-neutral-400 dark:border-neutral-700 last:border-r-0
                                                                    flex items-center justify-center text-xs font-bold
                                                                    touch-manipulation active:scale-95 transition-all duration-150
                                                                    ${isSelected ? 'bg-primary-600 text-white' : 'text-neutral-300 dark:text-neutral-600 hover:bg-neutral-100 dark:hover:bg-neutral-700'}
                                                                `}
            ...
            {/* Gráfico Estadístico */}
            <div className="flex p-4 justify-center items-center bg-white dark:bg-dark-bg-card border border-neutral-200 dark:border-dark-border shadow-sm">
                <BarChartCanvas data={chartData} />
            </div>

            {/* Leyenda (sin cambios) */}
            <div className="mt-6 bg-white dark:bg-dark-bg-card border border-neutral-200 dark:border-dark-border shadow-sm p-2 md-p-4">
                <div className="text-xs font-bold text-neutral-800 dark:text-dark-text-primary mb-2">Niveles de Logro</div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[10px] md:text-xs">
                    <div className="flex items-center space-x-2">
                        <span className="bg-red-100 text-red-700 px-1.5 py-0.5 font-bold border border-red-700 dark:bg-red-900/50 dark:text-red-300 dark:border-red-700">C</span>
                        <span className="text-neutral-700 dark:text-dark-text-secondary">En inicio</span>
                    </div>
                    <div className="flex items-center space-x-2">
                        <span className="bg-yellow-100 text-yellow-700 px-1.5 py-0.5 font-bold border border-yellow-700 dark:bg-yellow-900/50 dark:text-yellow-300 dark:border-yellow-700">B</span>
                        <span className="text-neutral-700 dark:text-dark-text-secondary">En proceso</span>
                    </div>
                    <div className="flex items-center space-x-2">
                        <span className="bg-blue-100 text-blue-700 px-1.5 py-0.5 font-bold border border-blue-700 dark:bg-blue-900/50 dark:text-blue-300 dark:border-blue-700">A</span>
                        <span className="text-neutral-700 dark:text-dark-text-secondary">Logro esperado</span>
                    </div>
                    <div className="flex items-center space-x-2">
                        <span className="bg-green-100 text-green-700 px-1.5 py-0.5 font-bold border border-green-700 dark:bg-green-900/50 dark:text-green-300 dark:border-green-700">AD</span>
                        <span className="text-neutral-700 dark:text-dark-text-secondary">Logro destacado</span>
                    </div>
                </div>
            </div>
        </div>
